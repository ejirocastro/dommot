...
const EkoAtlanticStaysPage: React.FC = () => {
    ...
    const sortedListings = useMemo(() => {
        const sorted = [...ekoAtlanticListings];
        switch (sortBy) {
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-16">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <h1 className="text-lg font-semibold text-gray-900">Stays in Eko Atlantic</h1>
                        </div>
                        <div className="flex items-center space-x-3">
                            <select value={sortBy} onChange={(e) => setSortBy(e.target.value as 'price' | 'rating' | 'newest')} className="text-sm border border-gray-300 rounded-lg px-3 py-2">
                                <option value="rating">Highest Rated</option><option value="price">Lowest Price</option><option value="newest">Newest</option>
                            </select>
                            <button className="flex items-center space-x-2 text-gray-600 hover:text-gray-900 border border-gray-300 rounded-lg px-3 py-2">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Stays in Eko Atlantic, Lagos</h2>
                            <p className="text-gray-600">{sortedListings.length} modern properties available</p>
                        </div>
                    </div>
                    <div className="flex items-center space-x-1 text-sm text-gray-600">
                        <Star className="w-4 h-4 text-yellow-400 fill-current" />
                        <span className="font-medium">{sortedListings.length > 0 ? (sortedListings.reduce((sum, listing) => sum + listing.rating, 0) / sortedListings.length).toFixed(2) : '0.00'}</span>
                ...
                {sortedListings.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {sortedListings.map((listing, index) => (
                            <div key={listing.id} className="w-full">
                                <ListingCard listing={listing} index={index} currentImageIndex={currentImageIndex[listing.id] || 0} isFavorite={favorites.has(listing.id)}
                                    onImageNext={() => handleImageNext(listing.id)} onImagePrev={() => handleImagePrev(listing.id)} onToggleFavorite={() => toggleFavorite(listing.id)} />
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-600">Check back soon for modern Eko Atlantic properties.</p>
                    </div>
                )}
            </div>
        </div>
...
export default EkoAtlanticStaysPage;
