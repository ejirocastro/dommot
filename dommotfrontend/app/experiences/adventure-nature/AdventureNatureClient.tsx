...
export default function AdventureNatureClient() {
    ...
    const categoryExperiences = useMemo(() => {
        return experiences.filter(exp => exp.category === 'Adventure & Nature');
    }, []);
    ...
    const toggleFavorite = (experienceId: number) => {
        setFavorites(prev => {
            ...
            if (newFavorites.has(experienceId)) newFavorites.delete(experienceId);
            else newFavorites.add(experienceId);
            return newFavorites;
        });
    };
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-16">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <h1 className="text-lg font-semibold text-gray-900">Adventure & Nature Experiences</h1>
                        </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Adventure & Nature Experiences</h2>
                            <p className="text-gray-600">{sortedExperiences.length} experiences available</p>
                        </div>
                    </div>
                    <div className="flex items-center space-x-1 text-sm text-gray-600">
                        <Star className="w-4 h-4 text-yellow-400 fill-current" />
                        <span className="font-medium">{sortedExperiences.length > 0 ? (sortedExperiences.reduce((sum, exp) => sum + exp.rating, 0) / sortedExperiences.length).toFixed(2) : '0.00'}</span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {sortedExperiences.map((experience, index) => (
                            <div key={experience.id} className="w-full">
                                <ExperienceCard experience={experience} index={index} currentImageIndex={currentImageIndex[experience.id] || 0}
                                    isFavorite={favorites.has(experience.id)} onImageNext={() => handleImageNext(experience.id)}
                                    onImagePrev={() => handleImagePrev(experience.id)} onToggleFavorite={() => toggleFavorite(experience.id)} />
                            </div>
                        ))}
                    </div>
                ) : (
                ...
                {sortedExperiences.length > 0 && (
                    <div className="mt-12 text-center">
                        <button className="bg-gray-900 text-white px-6 py-3 rounded-lg hover:bg-gray-800 transition-colors font-medium">Load More Experiences</button>
                    </div>
                )}
            </div>
        </div>
