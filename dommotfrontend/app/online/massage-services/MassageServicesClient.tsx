...
export default function MassageServicesClient() {
    const router = useRouter();
    const [currentImageIndex, setCurrentImageIndex] = useState<Record<number, number>>({});
    const [favorites, setFavorites] = useState<Set<number>>(new Set());
    const [sortBy, setSortBy] = useState<'price' | 'rating' | 'newest'>('rating');
    const categoryServices = useMemo(() => onlineServices.filter(svc => svc.category === 'Massage Services'), []);
        ...
        <div className="min-h-screen bg-white">
            ...
            <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-gray-200">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-16">
                        ...
                        <div className="flex items-center space-x-2"><MapPin className="w-5 h-5 text-sky-600" /><h1 className="text-lg font-semibold text-gray-900">Massage Services</h1></div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-4"><span className="text-2xl">💆‍♀️</span><div><h2 className="text-2xl font-bold text-gray-900">Massage Services</h2><p className="text-gray-600">{sortedServices.length} services available</p></div></div>
